...
void send_respond_head_common(int code, std::string& content_type,
	...
	sprintf(head, "%sContent-type: %s\r\n", head, content_type.c_str());
	char time_buf[32];
	timet_convert_to_chars(file_info.st_mtim.tv_sec, time_buf, sizeof(time_buf));
...
ew_http_respond_class::ew_http_respond_class(std::string& filename_s,
	...
	if (rs == -1) {
		...
		return;
	} else if (compare_time(file_info.st_mtim.tv_sec, rq_time) == 0) {
		//fprintf(stderr,"%d %d\n",file_info.st_mtim.tv_sec,rq_time);
		respond_code = EW_HTTP_NOT_MODIFIED;
	} else {
		//fprintf(stderr,"%d %d\n",file_info.st_mtim.tv_sec,rq_time);
		respond_code = EW_HTTP_OK;
	}
	file_fd = open(filename.c_str(), O_RDONLY);
	content_type = std::string(find_type(filename_s.c_str()));
}
...
void ew_http_request_class::http_request_head_parser(ew_init_config& conf) {
	...
	const char* tmp_pos;
	if ((tmp_pos = strstr(pos, "If-Modified-Since"))) {
		tmp_pos = tmp_pos + strlen("IF-Modified-Since");
